...
| Date (UTC) | Change |
|---|---|
| 2026-06-07 | **Open positions from the broker, not derived.** `.tlg` `STK_LOT`/`OPT_LOT`/`FUT_LOT` rows (IB open-lot snapshot) are parsed into a `Position[]` (aggregated per account+symbol, weighted-avg cost, signed qty), persisted to a Dexie `positions` table (v5), and shown verbatim in the Positions view — instead of inferring open positions from reconstructed-trade `isOpen`. Snapshot semantics: an import replaces the position set only when it carries one (CSV/Trade Republic don't). Cleared by wipe-trades / wipe-all; included in JSON export. Engineering: code-split ECharts (tree-shaken core + lazy chart views → entry ~350 kB, no >500 kB warning), virtualized Trades table, property-based reconstruction test (fast-check), PWA/offline (vite-plugin-pwa). |
